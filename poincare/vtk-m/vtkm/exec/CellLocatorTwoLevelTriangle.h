...
//============================================================================
#ifndef vtk_m_exec_CellLocatorTwoLevelTriangle_h
#define vtk_m_exec_CellLocatorTwoLevelTriangle_h
...
namespace internal
{
namespace cl_uniform_bins_triangle
{
...
} // vtkm::internal::cl_uniform_bins_triangle
...
class VTKM_ALWAYS_EXPORT CellLocatorTwoLevelTriangle
{
private:
  using DimVec3 = vtkm::internal::cl_uniform_bins_triangle::DimVec3;
  using FloatVec3 = vtkm::internal::cl_uniform_bins_triangle::FloatVec3;
  ...
  VTKM_EXEC static vtkm::ErrorCode PointInsideCell(FloatVec3 point,
                                                   ...
                                                   bool& inside)
  {
    auto bounds = vtkm::internal::cl_uniform_bins_triangle::ComputeCellBounds(cellPoints);
    if (point[0] >= bounds.Min[0] && point[0] <= bounds.Max[0] && point[1] >= bounds.Min[1] &&
        point[1] <= bounds.Max[1]) //2D only && point[2] >= bounds.Min[2] && point[2] <= bounds.Max[2])
    {
      ...
      /*
      auto pc = parametricCoordinates;
      auto i2 = inside;
      VTKM_RETURN_ON_ERROR(vtkm::exec::WorldCoordinatesToParametricCoordinates(
        cellPoints, point, cellShape, pc));
      i2 = vtkm::exec::CellInside(pc, cellShape);
      if (inside != i2 || vtkm::Magnitude(pc-parametricCoordinates) > 1e-12)
        std::cout<<"***************************************   CLoc ERROR! "<<vtkm::Magnitude(pc-parametricCoordinates)<<std::endl;
      */
    }
    else
    {
...
public:
  ...
  VTKM_CONT CellLocatorTwoLevelTriangle(const vtkm::internal::cl_uniform_bins_triangle::Grid& topLevelGrid,
...
vtkm::ErrorCode FindCell2(const FloatVec3& point,
  ...
  {
    using namespace vtkm::internal::cl_uniform_bins_triangle;
...
//    std::cout<<" ***** No find: "<<prevCell<<std::endl;
  ...
  VTKM_EXEC CellLocatorTwoLevelTriangle* operator->() { return this; }
  VTKM_DEPRECATED(1.6, "Locators are no longer pointers. Use . operator.")
  VTKM_EXEC const CellLocatorTwoLevelTriangle* operator->() const { return this; }

private:
  vtkm::internal::cl_uniform_bins_triangle::Grid TopLevel;
...
#endif //vtk_m_exec_CellLocatorTwoLevelTriangle_h
